...
from swb.mlops.db.core.db_api_client import execute_post_request, execute_post_request_for_files
from swb.mlops.db.utils.paths import create_full_target_path


def db_wkspace_mdkir(db_workspace_uri: str, db_token: str, path_to_mkdir: str):
    """
...
def db_wkspace_import(db_workspace_uri: str, db_token: str, source_file: str, target_file: str, language='PYTHON'):
    """
...
def notebook_copy(db_workspace_uri: str, db_token: str, source_nb_dir: str, target_nb_dir: str, language='PYTHON'):
    """
    ...
    for root, dirs, files in os.walk(source_nb_dir):
        ...
        for f in files:
            if f.endswith('.{}'.format(const.file_format['PYTHON'])):
...
def db_workspace_dir_delete(db_workspace_uri: str, db_token: str, target_nb_dir: str):
    """
    This function deletes directories in workspaces

    :param db_workspace_uri: The Databricks workspace URI to use for this API call.
    :param db_token: The Databricks personal token to use for this API call.
    :param target_nb_dir: The workspace path to delete from.  Ex: /Shared/use_case/...
    ...
    uri = db_workspace_uri + "/api/2.0/workspace/delete"
    json_dict = {
        "path": "%s" % target_nb_dir,
        "recursive" : True
    }
    print("Deleting workspace folder : {}".format(target_nb_dir))
    response = execute_post_request(uri, db_token, json_dict_input=json_dict)
    return response
